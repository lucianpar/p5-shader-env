...
void main() {
      vec2 fragUv = uv * 5.0 - 1.0;

    float t = u_time*0.1;

    float k = cos(t);
    float l = sin(t);
    float s = 1.0 + (0.0000000000001*u_time); //+ (onset / u_time);

    for(int i=0; i<30; ++i) {
        fragUv.x = abs(fragUv.x) - s ;// for miiror - could remove
     
        fragUv *= mat2(k*1.001,l*1.00001,l*0.9999999999999,-k*0.99); // for rotation - keep subtle
        // s  *= .999;///(t+1);         // Scale
    }
    ...
    vec3 normal = normalize(vec3(
        shade - noise(fragUv + vec2(0.01, 0.0)),
